...
const { data: trackData } = await useFetch<{ data: Track }>(
  "https://discovery-us-01.audius.openplayer.org/v1/tracks/" + route.params.id,
  {
    query: {
      app_name: "GENESIS-TM",
    },
...
<template>
  ...
  >
    <img
      class="absolute h-32 w-32 rounded-full bottom-0 left-10 translate-y-5 z-10 overflow-auto"
      :src="trackData?.data?.artwork?.['480x480'] || '/music-banner.jpg'"
      alt="Cover Image"
    />
    <h1
      class="absolute bottom-12 left-44 font-bold text-white glass p-1 rounded-lg text-xl lg:text-3xl"
    >
